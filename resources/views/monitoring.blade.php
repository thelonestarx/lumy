    ...
    <aside :class="sidebarOpen ? 'w-64 p-6' : 'w-20 p-4'" class="bg-blue-900 text-white flex flex-col justify-between transition-all duration-300 ease-in-out shrink-0">
        <div>
            ...
            <nav class="space-y-2">
                ...
                <a href="{{ route('monitoring') }}" :class="sidebarOpen ? 'justify-start' : 'justify-center'" class="flex items-center gap-3 bg-blue-800 text-white p-3 rounded-xl font-medium transition-all" title="Monitoring">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 mb-6">
            <div class="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
                ...
                <div class="flex items-center gap-3">
                    <select class="border border-slate-200 rounded-lg text-sm px-3 py-1.5 text-slate-600 focus:outline-none">
                    ...
                    <div class="flex bg-slate-100 p-1 rounded-lg text-xs font-semibold text-slate-600">
                        <button class="px-3 py-1 rounded-md bg-white shadow-sm text-blue-900">1 Hari</button>
                        <button class="px-3 py-1 rounded-md hover:text-slate-900">1 Minggu</button>
                        <button class="px-3 py-1 rounded-md hover:text-slate-900">1 Bulan</button>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <div class="flex justify-between items-center mb-4">
                ...
                <button class="text-xs font-semibold text-blue-900 hover:underline flex items-center gap-1">
                    <i class="fa-solid fa-download"></i> Unduh Data Log
                </button>
            </div>
                ...
                <table class="w-full text-left text-sm text-slate-600">
                    <thead class="bg-slate-50 text-slate-500 text-xs uppercase font-semibold border-b border-slate-100">
                        <tr>
                            ...
                            <th class="py-3 px-4">Tingkat pH</th>
                            <th class="py-3 px-4">Kekeruhan (NTU)</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50/80 transition">
                            <td class="py-3 px-4 font-medium text-slate-800">{{ $log['time'] }}</td>
                            <td class="py-3 px-4">{{ $log['suhu'] }} °C</td>
                            <td class="py-3 px-4">{{ $log['ph'] }}</td>
                            <td class="py-3 px-4">{{ $log['turbidity'] }} NTU</td>
                            <td class="py-3 px-4">
                                @if($log['status'] == 'Normal')
                                <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-600 border border-emerald-200">Normal</span>
                                @elseif($log['status'] == 'Waspada')
                                <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-50 text-amber-600 border border-amber-200">Waspada</span>
                                @else
                                <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-rose-50 text-rose-600 border border-rose-200">Darurat</span>
                                @endif
                            </td>
                        </tr>
                        @endforeach
